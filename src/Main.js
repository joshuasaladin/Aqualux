...
function handleSubmission_(sub, message) {
  ...
  if (!sub.email) {
    markReview_(message, 'Submission has no guest email address.');
  ...
  if (isContactForm_(sub.formName)) {
    markReview_(message, 'Contact form message from ' + (base.guestName || sub.email) + '.');
  ...
  if (isDuplicateSubmission_(sub)) {
    markReview_(message, 'Duplicate submission from ' + sub.email + ' for ' + sub.formName + '.');
  ...
  if (!service) {
    ...
    markReview_(message, 'Form "' + sub.formName + '" is not in the catalog.');
  ...
  if (!match.variant) {
    ...
    if (service.attachMenu) {
      sendMenuOptions_(sub, service, message, base);
      return;
    }
    const asks = [];
    if (match.candidates) {
    ...
    sendClarification_(sub, service, asks, message, base);
    return;
  }
  ...
  if (pricing.status === 'manualQuote') {
    ...
    markReview_(message, service.name + ': draft ready — fill in the $____ amount before sending.');
    appendLog_(Object.assign(base, {
  ...
  if (pricing.status === 'inquire') {
    ...
    const mode = deliverGuestEmail_(sub.email, email, null);
    markReview_(message, service.name + ' (' + variant.name + '): price quoted, awaiting guest confirmation + booking details.');
    appendLog_(Object.assign(base, {
  ...
  if (pricing.status === 'clarify' || preMissing.length) {
    ...
    sendClarification_(sub, service, missing, message, base);
    return;
  }
...
function sendClarification_(sub, service, missing, message, base) {
...
function sendMenuOptions_(sub, service, message, base) {
